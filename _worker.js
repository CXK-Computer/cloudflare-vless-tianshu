/**
 * VLESS & Trojan 双协议 Worker (最终修复版)
 * 该脚本通过硬编码 Trojan 密码，在 Worker 启动时预计算哈希值，
 * 彻底解决了因运行时计算哈希导致的 CPU 超时和 1101 错误。
 * 它专注于提供最高性能和最干净 TLS 指纹的直连代理。
 * @version 5.1.0
 */
...
// !!! 重要：请在这里修改您的 Trojan 密码 !!!
const TROJAN_PASSWORD = "aaeaa2f3-1a94-46fb-8438-23f46052a0fa"; // 您可以设置为任何字符串

// ==================
//   全局变量与预计算
// ==================

// 在 Worker 实例启动时，预先计算好 Trojan 密码的哈希值
// 这是解决 1101 错误的关键
const TROJAN_EXPECTED_HASH_BYTES = precomputeTrojanHash(TROJAN_PASSWORD);
...
class Config {
    constructor(env) {
        // VLESS UUID 仍然通过环境变量配置，因为它不涉及高消耗的计算
        this.vlessId = env.UUID || "ef3dcc57-6689-48e4-b3f9-2a62d88c730a";
        this.trojanPassword = TROJAN_PASSWORD; // 使用预设的密码
        this.validate();
    }
    validate() {
...
export default {
    ...
    async handleWebSocket(webSocket, request, config) {
        ...
        if (headerResult.error) {
            ...
            if (headerResult.error) {
                logToUI('error', '协议认证失败', { error: '既不是 VLESS 也不是 Trojan', details: headerResult.error, connectionId });
        ...
        logToUI('info', '协议认证成功', { protocol, target: `${address}:${port}`, connectionId });
