 ...
 * @version 3.0.3 - 修复了因 Trojan 哈希计算导致 CPU 超时从而引发 1101 错误的性能问题。
 */
...
class TrojanProtocolHandler {
    constructor(expectedHashBytes) {
        this.expectedHashBytes = expectedHashBytes;
    }
...
export default {
    ...
    generateUIPage(request, config) {
      ...
      <body>
          ...
          <script>function copyToClipboard(e){const o=document.getElementById(e).innerText;navigator.clipboard.writeText(o).then(()=>{const e=document.getElementById("copy-status");e.style.display="block",setTimeout(()=>{e.style.display="none"},2e3)})}</script>
      </body>
...
function precomputeTrojanHash(password) {
    // Pure JS SHA-224 implementation, moved to a separate function
    const K = [
